...
TEST_F(CliTest, ParseArgs_UnknownFlag_ThrowsInvalidArgument) {
    const std::array<std::string_view, 2> argv = {"/tmp/logs"sv, "--unknown"sv};
    EXPECT_THROW(
        std::ignore = ParseArgs(std::span<const std::string_view>{argv}),
        std::invalid_argument);
}

TEST_F(CliTest, ParseArgs_KeywordsFlagWithoutValue_ThrowsInvalidArgument) {
    const std::array<std::string_view, 2> argv = {"/tmp/logs"sv,
                                                  "--keywords"sv};
    EXPECT_THROW(
        std::ignore = ParseArgs(std::span<const std::string_view>{argv}),
        std::invalid_argument);
...
TEST_F(CliTest, ParseArgs_JsonFlagWithoutValue_ThrowsInvalidArgument) {
    const std::array<std::string_view, 2> argv = {"/tmp/logs"sv, "--json"sv};
    EXPECT_THROW(
        std::ignore = ParseArgs(std::span<const std::string_view>{argv}),
        std::invalid_argument);
}
